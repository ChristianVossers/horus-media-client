...
DEVICE = 'cuda:0'
sam = sam_model_registry["vit_h"](checkpoint='horus_media_examples/GeoAI/models/sam_V1_Huge.geoAI').to(device=DEVICE)
...
def processSAM_V1(cv2_image, bboxes, multiMask):
...
def processSAM_V1_All(cv2_image, bboxes, outputFileName, multiMask=False):
    bboxes = torch.tensor(bboxes, device=DEVICE)
    masks, _, _ = processSAM_V1(cv2_image=cv2_image,bboxes=bboxes,multiMask=multiMask)
...
if __name__ == "__main__":
    ...
    processSAM_V1_All(cv2_image=cv2_image,bboxes=bboxes, outputFileName='horus_media_examples/GeoAI/output/test.png', multiMask=False)
